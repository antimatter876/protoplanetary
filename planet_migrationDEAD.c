...
int main()
{
  ...
  static double R_s = 4;      /* AU - initial disc radius-Changed from 10AU */
  ...
  double Courant_fix;
  /* GRID & VISC */
  double Rz[10001],X[10001],nu[10001],Rf[10001],XX[10001],alpha[10001];
  /* GAS */
  ...
  for (i=in; i<out+1; i++)
    {
      H[i] = H_R * pow(Rz[i],q);
      alpha[i]= 0.01 * (1.0/(1+exp((Rz[i]-1)/0.3)) + 1.0/(1+exp(-(Rz[i]-7)/0.3))) + 0.0001;
      nu[i]= alpha[i] * H[i] * H[i] * Omega[i];  /* AU^2/yr */
    }
